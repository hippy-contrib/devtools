...
const socket = function initSocket(url, handlers, reconnect) {
  ...
  client.onMessage((data) => {
    ...
    //   doReport(hmrData.performance, data.length)
    // }
...
function doReport(performance, length) {
  try {
    import('@tencent/aegis-hippy-sdk').then(({default: Aegis}) => {
      ...
      const { pcToServer, serverToApp } = performance;
      const report = (name, start, end) => {
        if(start && end)
          aegis.reportTime({
            name: name,
            duration: end - start,
            ext1: Math.ceil(length/1024) + 'KB',
          });
      }
      report('HMR-server-to-app', serverToApp, appReceive);
      report('HMR-total', pcToServer, appReceive);
    })
  } catch(e) {}
};
