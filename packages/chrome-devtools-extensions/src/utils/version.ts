 ...
 */
export const compareVersionLatest = (v1: string, v2: string): VersionResult => {
    ...
    if (v1PreRelease && !v2PreRelease) {
      ...
      if (gama1 > gama2) return VersionResult.GreaterThan
      if (gama2 > gama1) return VersionResult.LessThan
      if (parseInt(time1) > parseInt(time2)) return VersionResult.GreaterThan
      if (parseInt(time2) > parseInt(time1)) return VersionResult.LessThan
    }
    return VersionResult.EqualTo
  }